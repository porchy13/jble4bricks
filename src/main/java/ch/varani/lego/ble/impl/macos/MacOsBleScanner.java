...
import ch.varani.lego.ble.api.BleScanner;
import ch.varani.lego.ble.api.ScanCallback;
import ch.varani.lego.ble.util.NativeLibraryLoader;
 ...
 * {@link NativeBridge#init()} to allocate the underlying
 ...
 *
 * <p>The native side calls back into {@link #onDeviceFound(String, String, int)}
 * for every discovered peripheral.  CoreBluetooth delivers those callbacks on
 * its own internal dispatch queue, not on a JVM thread, so the JNI bridge
 ...
 * @since 1.0
 */
public final class MacOsBleScanner implements BleScanner {
    ...
    CompletableFuture<MacOsBleConnection> connectPeripheral(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            return new MacOsBleConnection(connPtr, ptr, this);
        }, exec);
    }

    /**
     * Disconnects the given native connection context and frees its resources.
     *
    ...
    /* ─────────────────────────────────────────────────────────────────────────
       JNI callback — called by the native layer on each scan result
       ───────────────────────────────────────────────────────────────────────── */

    /**
     * Called by the native layer (from the CoreBluetooth dispatch queue) each
     * time a BLE advertisement is received.
     *
     * <p>The corresponding native invocation is in {@code BleBridge.m},
