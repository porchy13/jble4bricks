...
/**
 * Linux implementation of {@link BleConnection} backed by the BlueZ D-Bus API
 * ({@code org.bluez}) via JNI.
 *
 * <p>Instances are created by
 * {@link LinuxBleScanner#connectDevice(String)} after the native layer has
 * called {@code org.bluez.Device1.Connect} and completed GATT service and
 ...
 * calls {@link #onNotification(String, String, byte[])} from the GLib
 * main-loop thread (a JNI-attached background thread), so the publisher must
 * be safe for concurrent submission.
 *
 * @since 1.0
 */
final class LinuxBleConnection implements BleConnection {

    private static final Logger LOG = Logger.getLogger(LinuxBleConnection.class.getName());
    ...
    private final LinuxBleScanner scanner;

    /** The device this connection belongs to. */
    private final LinuxBleDevice device;
    ...
    /**
     * Constructs a new {@code LinuxBleConnection}.
     *
     ...
     * @param scanner       the owning {@link LinuxBleScanner}
     */
    LinuxBleConnection(
            final long connectionPtr,
            final long contextPtr,
            final @NonNull LinuxBleScanner scanner) {
     ...
     * {@link LinuxBleDevice}.  Used by tests.
     *
     * @param connectionPtr opaque pointer to the native {@code BleConnectionContext}
     * @param contextPtr    opaque pointer to the native {@code BleContext}
     * @param scanner       the owning {@link LinuxBleScanner}
    ...
    LinuxBleConnection(
            ...
            final long contextPtr,
            final @NonNull LinuxBleScanner scanner,
            final @NonNull LinuxBleDevice device) {
        this.connectionPtr = connectionPtr;
        this.contextPtr    = contextPtr;
        this.scanner       = scanner;
        this.device        = device;
     ...
     *
     * <p>Delegates to
     * {@link LinuxBleScanner#writeWithoutResponseNative(long, String, String, byte[])}
     * which calls {@code org.bluez.GattCharacteristic1.WriteValue} with the
     ...
     *
     * <p>Calls
     * {@link LinuxBleScanner#readCharacteristicNative(long, String, String)}
     * which invokes {@code org.bluez.GattCharacteristic1.ReadValue} over D-Bus
     * and blocks until the peripheral delivers the value.
     */
    @Override
     ...
     * native side via {@code org.bluez.GattCharacteristic1.StopNotify}, and
     * calls {@link LinuxBleScanner#disconnectNative(long)}.
     */
    @Override
    ...
    /**
     * Called by {@link LinuxBleScanner#onNotification} when a GATT
     ...
     * for {@code org.bluez.GattCharacteristic1} in {@code BleBridge.c}.
     *
