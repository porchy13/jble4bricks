...
final class MacOsBleDevice implements BleDevice {
    ...
    private static final Logger LOG = Logger.getLogger(MacOsBleDevice.class.getName());
     ...
     * @param manufacturerData raw manufacturer-specific payload bytes (copied); must not be
     *                      {@code null}
     * @param owner         the scanner instance that discovered this device; must not be
     *                      {@code null}
     */
    MacOsBleDevice(
     ...
     * extracted from the CoreBluetooth advertisement data dictionary key
     * {@code CBAdvertisementDataManufacturerDataKey}.
     */
    @Override
    ...
    public @NonNull CompletableFuture<BleConnection> connect() {
        LOG.info(() -> "Initiating connection to device: id=" + peripheralId
                + " name='" + deviceName + "'");
